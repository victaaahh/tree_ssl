...
class Random3AxisRotation(object):
    ...
    def __init__(self, apply_rotation: bool = True, rot_x: float = None, rot_y: float = None, rot_z: float = None,
        ...
        self._rot_x = np.abs(min(rot_x, 180)) if rot_x else 0
        self._rot_y = np.abs(min(rot_y, 180)) if rot_y else 0
        self._rot_z = np.abs(min(rot_z, 180)) if rot_z else 0
    ...
    def generate_random_rotation_matrix(self):
        ...
        for axis_ind, deg_angle in enumerate(self._degree_angles):
            if deg_angle > 0 and random.random() < self._p:
...
class AddXYDistanceToCenter(object):
    """
    Distance to a certain point (center)
    """

    def __init__(self, center_x: float, center_y: float):
        self.pdist = PairwiseDistance()
        self.center: torch.Tensor = torch.tensor([[center_x, center_y]])

    def __call__(self, data):
        pos = data.pos[:, :2]

        data.xy_distance = self.pdist(pos, self.center.repeat_interleave(pos.shape[0], dim=0))
        return data

    def __repr__(self):
        return "{}(center_x: {}, center_y: {})".format(self.__class__.__name__, self.center[0, 0], self.center[0, 1])


class AddZDistanceToTop(object):
    """
    Add distance to top of the point cloud (99 quantile)
    """

    def __call__(self, data):
        pos = data.pos[:, [2]]
        highest_point = torch.quantile(pos, 0.99, keepdim=True)

        data.z_distance_to_top = -(pos - highest_point)
        return data
